...
function check(){
  ...
  if ((lastscreennum!=1 || lastu2) && !u2){
    document.getElementById("u2").innerHTML="<button onclick='if (infinitypoints>=50){u2=1;infinitypoints-=50;u2l=setInterval(autometa,100);}'>Get metapoint autoclicker, 50 IP</button><br>";
  ...
  if ((lastscreennum!=1 || lastu3) && !u3){
    document.getElementById("u3").innerHTML="<button onclick='if (infinitypoints>=100){u3=1;infinitypoints-=100;u3l=setInterval(autometameta,100);}'>Get metametapoint autoclicker, 100 IP</button><br>";
  ...
  if ((lastscreennum!=1 || lastu4) && !u4){
    document.getElementById("u4").innerHTML="<button onclick='if (infinitypoints>=250){u4=1;infinitypoints-=250;u4l=setInterval(autoIP,100);}'>Get Infinity autoclicker, 250 IP</button><br>";
...
function play(num){
  if (energy>=5 && eval("gdlvl"+num)==0){
    ...
    var chance=1;
    for (i=1; i<=7; i++){
      trychance=eval("gdsk"+i)/(100*gdlvldata[num-1][i-1]+1)
      if (trychance<1){
        chance*=trychance;
      }
    }
